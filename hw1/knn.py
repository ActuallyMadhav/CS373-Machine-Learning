...
class KNearestNeighbor:
    ...
    Attributes:
        ...
        weighted (bool): If True, neighbors vote with weight 1 / (distance + 1e-8)
            instead of one vote each (distance-weighted KNN).
    """
    
    def __init__(self, k: int, weighted: bool = False) -> None:
        """
        The constructor for KNearestNeighbor class. Saves the k value to
        `self.k` and the voting scheme to `self.weighted`. Initializes the
        training data and labels to `None`.

        Parameters:
            k (int): The number of nearest neighbors to consider.
            weighted (bool): Whether to use distance-weighted voting. Default False.
        ...
        Examples:
            ...
            >>> knn.X_train
            >>> knn.y_train
            >>> KNearestNeighbor(3, weighted=True).weighted
            True
        """
    ...
    def calc_distance(self, x, p=4):
        ...
        p (int): The order of the norm. Default is 4 (the order used in this homework).
    ...
    def get_top_k(self, distance: np.ndarray) -> np.ndarray:
        ...
        Parameters:
            distance (np.ndarray): The Minkowski distance between the training 
    ...
    def predict(self, X_predict: np.ndarray, p: int = 4) -> np.ndarray:
        """
        This function predicts the labels for the test data by calculating the
        Minkowski distance between the training data and the test data, and
        finding the top k smallest distances.

        If `self.weighted` is False, the predicted label is the label that
        appears the most among the top k neighbors (each neighbor has one vote).

        If `self.weighted` is True, each neighbor votes with weight
        1 / (distance + 1e-8), and the predicted label is the label with the
        largest total weight.

        You may find `np.argmax` and `np.bincount` (with its `weights` argument)
        useful.
        ...
        Examples:
            ...
            >>> knn.predict(X_predict, 4)
            array([1])
            >>> X_train = np.array([[0, 0], [3, 0], [3, 0.1], [3, -0.1]])
            >>> y_train = np.array([0, 1, 1, 1])
            >>> knn = KNearestNeighbor(3)
            >>> knn.fit(X_train, y_train)
            >>> knn.predict(np.array([[0.5, 0]]), 4)
            array([1])
            >>> knn = KNearestNeighbor(3, weighted=True)
            >>> knn.fit(X_train, y_train)
            >>> knn.predict(np.array([[0.5, 0]]), 4)
            array([0])
        """
...
if __name__ == "__main__":
    ...
    from utils_knn import print_green, print_red
